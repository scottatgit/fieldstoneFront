...
export default function SecurityPage() {
  ...
  useEffect(() => {
    if (isLoaded && user?.role === 'platform_admin') void loadStatus();
  }, [isLoaded, user, loadStatus]);

  async function startSetup() {
    setBusy(true); setError(''); setQrDataUrl(''); setShowUri(false); setCopied(false);
    try {
      const d = await adminFetchDirect('/api/auth/mfa/setup', { method: 'POST' }) as
        ...
        <HStack justify="space-between">
          <VStack align="start" spacing={0}>
            ...
            <Text fontSize="xs" color="gray.500" fontFamily="mono">
              Platform admin authentication settings
            </Text>
          </VStack>
