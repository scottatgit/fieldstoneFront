...
/**
 * Platform Docs Page
 * Access: platform_admin only (enforced by /app/platform/layout.tsx)
 * Moved from /pm/docs — tenant/user nav link removed per docs-to-platform migration.
 */
...
function S1() {
  ...
  const caps = [
    ...
    { icon: '📋', label: 'PM Dashboard',                      note: 'tickets, signals, intel, setup' },
  ];
  return (
    <Box>
          ...
          <HStack spacing={8} justify="center" flexWrap="wrap">
            <VStack spacing={0}>
              ...
              <Text fontSize="9px" color="gray.500" fontFamily="monospace" mt={1}>
                /pm · /pm/intel · /pm/setup · /platform/docs
              </Text>
            </VStack>
            <VStack spacing={0}>
...
function S9() {
  const steps = [
    ...
    { n: 6, title: 'Test Connections',           detail: 'Use the Test buttons for AI and Email to verify configuration before configuration before saving.' },
...
export default function PlatformDocsPage() {
  ...
  // Platform layout (platform/layout.tsx) enforces platform_admin auth.
  // No SummaryBar or DemoBanner needed here — PlatformNav is injected by layout.
  return (
    <Box minH="100dvh" bg={bg} color={textColor} overflowX="hidden">
      <Flex h="calc(100dvh - 44px)" overflow="hidden">
        ...
        <Box flex={1} overflowY="auto" p={{ base: 4, md: 8 }}>
          ...
          <Box mb={8}>
            <HStack mb={2}>
              ...
              <Badge colorScheme="orange" fontSize="sm">Platform Admin Only</Badge>
            </HStack>
