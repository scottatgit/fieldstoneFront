...
interface Workspace { tenant_id: string; slug: string; name: string; role: string; }

const ROLE_COLORS: Record<string, string> = { tenant_admin: 'blue', technician: 'green', viewer: 'gray' };
const ROLE_LABELS: Record<string, string> = { tenant_admin: 'Admin', technician: 'Tech', viewer: 'Viewer' };
...
export function WorkspaceSwitcher() {
  ...
  function switchTo(ws: Workspace) {
    ...
    window.location.href = isLocal ? '/pm' : `${window.location.protocol}//${ws.slug}.${SIGNAL_DOMAIN}/pm`;
    setOpen(false);
  }

  return (
    <Box position='relative' ref={dropdownRef}>
      <Flex align='center' px={3} minH='44px' gap={2}
        borderRight='1px solid' borderColor='gray.700'
        cursor='pointer' _hover={{ bg: 'gray.800' }}
        onClick={() => setOpen(o => !o)} userSelect='none'>
        <Text fontSize='xs' fontWeight='semibold' color='gray.300' fontFamily='mono' letterSpacing='wide'>{current.name}</Text>
              ...
              return (
                ...
                >
                  <VStack spacing={0} align='flex-start'>
                    <Text fontSize='sm' fontWeight={isCurrent ? 'bold' : 'medium'} color={isCurrent ? 'white' : 'gray.300'}>{ws.name}</Text>
                    <Text fontSize='xs' color='gray.600' fontFamily='mono'>{ws.slug + '.' + SIGNAL_DOMAIN}</Text>
                  </VStack>
                  <Flex align='center' gap={2}>
                    <Badge fontSize='2xs' colorScheme={ROLE_COLORS[ws.role] ?? 'gray'} variant='subtle'>{ROLE_LABELS[ws.role] ?? ws.role}</Badge>
